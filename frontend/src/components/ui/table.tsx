...
function Table({ className, ...props }: React.ComponentProps<"table">) {
  return (
    <div data-slot="table-container" className="relative w-full overflow-x-auto">
      <table data-slot="table" className={cn("w-full caption-bottom text-left text-[13px]", className)} {...props} />
    </div>
  );
}
...
function TableFooter({ className, ...props }: React.ComponentProps<"tfoot">) {
  return (
    <tfoot
      ...
      className={cn("border-t border-line bg-surface-2 font-medium [&>tr]:last:border-b-0", className)}
      {...props}
    />
  );
}
...
function TableCaption({ className, ...props }: React.ComponentProps<"caption">) {
  return <caption data-slot="table-caption" className={cn("mt-3 text-xs text-ink-faint", className)} {...props} />;
}
