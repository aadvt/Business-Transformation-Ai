...
export default function RegisterVendorPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <h1 className="font-display mb-2 text-[30px] leading-[1.15] font-bold tracking-[-0.02em] text-ink">
        List your business
      </h1>
      ...
      <form onSubmit={handleSubmit} className="panel space-y-5 p-6">
        {submitError && (
          <div className="flex items-start gap-2 rounded-md bg-critical-dim px-3.5 py-3 text-[13px] text-critical">
        ...
        <Field label="GSTIN">
          <input
            ...
            maxLength={15}
            className={`${inputClass} numeric ${gstinCheck && !gstinCheck.valid ? "border-critical" : gstinCheck?.valid ? "border-success" : ""}`}
          />
          {gstinCheck && !gstinCheck.valid && (
            <p className="mt-1.5 flex items-center gap-1.5 text-[11.5px] text-critical">
        ...
        <Field label="Udyam number (optional)" hint="UDYAM-XX-YY-NNNNNNN">
          <input value={udyam} onChange={(e) => setUdyam(e.target.value.toUpperCase())} className={`${inputClass} numeric`} />
        </Field>
        ...
        <Field label="Phone" hint="+91-XXXXX-XXXXX">
          <input
            ...
            className={`${inputClass} numeric ${touched && !phoneValid ? "border-critical" : ""}`}
          />
          {touched && !phoneValid && <p className="mt-1.5 text-[11.5px] text-critical">Expected format +91-XXXXX-XXXXX</p>}
        </Field>
        ...
        <Field label="Pincode">
          <input
            ...
            maxLength={6}
            className={`${inputClass} numeric ${touched && !pincodeValid ? "border-critical" : ""}`}
          />
          {touched && !pincodeValid && <p className="mt-1.5 text-[11.5px] text-critical">6-digit pincode required</p>}
        </Field>
        ...
        <Field label="Languages">
          ...
          {touched && languages.length === 0 && (
            <p className="mt-1.5 text-[11.5px] text-critical">Pick at least one language</p>
          )}
        </Field>
        ...
        <button
          ...
          className="w-full rounded-md bg-accent py-2.5 text-[13.5px] font-semibold text-accent-ink outline-offset-2 transition-colors duration-200 hover:bg-accent-bright disabled:cursor-not-allowed disabled:bg-line-strong disabled:text-ink-faint"
        >
